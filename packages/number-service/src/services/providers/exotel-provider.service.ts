 ...
 */
export class ExotelProvider extends BaseTelecomProvider {
  ...
  async searchNumbers(request: NumberSearchRequest): Promise<NumberSearchResponse> {
    ...
    try {
      ...
      logger.error('Exotel number search failed', { error, request });
  ...
  async reserveNumber(request: NumberReservationRequest): Promise<NumberReservationResponse> {
    ...
    logger.info('Exotel number reservation requested', { request, reservationId });
  ...
  async purchaseNumber(request: NumberPurchaseRequest): Promise<NumberPurchaseResponse> {
    ...
    try {
      ...
      logger.info('Exotel number purchase requested', {
        request,
        purchaseId,
      ...
      logger.error('Exotel number purchase failed', { error, request });
  ...
  async portNumber(request: PortingRequest): Promise<PortingResponse> {
    ...
    try {
      ...
      logger.info('Exotel porting request submitted', {
        request,
        portingId,
      ...
      logger.error('Exotel porting request failed', { error, request });
  ...
  async releaseReservation(reservationId: string): Promise<boolean> {
    logger.info('Exotel reservation released', { reservationId });
    return true;
  }
   ...
   */
  async makeCall(from: string, to: string, callerId?: string): Promise<any> {
    try {
      ...
      logger.info('Exotel call initiated', { from, to, response: response.data });
      ...
      logger.error('Exotel call failed', { error, from, to });
      throw error;
    }
  }
   ...
   */
  async sendSms(from: string, to: string, body: string, templateId?: string): Promise<any> {
    try {
      const params = new URLSearchParams({
        From: from,
        To: to,
      ...
      logger.info('Exotel SMS sent', { from, to, response: response.data });
      ...
      logger.error('Exotel SMS failed', { error, from, to });
      throw error;
    }
  }
  ...
  async getCallRecording(callSid: string): Promise<string | null> {
    try {
      ...
      logger.error('Failed to get Exotel recording', { error, callSid });
      return null;
    }
  }
  ...
  private extractIndianCircle(phoneNumber: string): string {
  ...
  private getCircleName(phoneNumber: string): string {
  ...
  private mapExotelFeatures(exoPhone: any): string[] {
    const features: string[] = ['voice', 'call_forwarding', 'ivr', 'analytics'];

    if (exoPhone.Capabilities?.voice) features.push('voice');
    if (exoPhone.Capabilities?.sms) features.push('sms');
    if (exoPhone.Capabilities?.whatsapp) features.push('whatsapp');
  ...
  private getSampleAvailableNumbers(city: string, limit: number): ProviderNumber[] {
