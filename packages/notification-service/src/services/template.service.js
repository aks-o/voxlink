...
class TemplateService {
    ...
    async createTemplate(input) {
        try {
            ...
            handlebars_1.default.compile(input.content);
            if (input.subject) {
                handlebars_1.default.compile(input.subject);
            }
            ...
            logger_1.logger.error('Failed to create notification template', {
    ...
    async renderTemplate(input) {
        try {
            ...
            const contentTemplate = handlebars_1.default.compile(template.content);
            ...
            logger_1.logger.debug('Template rendered successfully', {
    ...
    async updateTemplate(templateId, updates) {
        try {
            ...
            if (updates.content) {
                handlebars_1.default.compile(updates.content);
            }
            if (updates.subject) {
                handlebars_1.default.compile(updates.subject);
            }
            const template = await this.prisma.notificationTemplate.update({
                where: { id: templateId },
                data: updates,
            });
            logger_1.logger.info('Notification template updated', {
                templateId,
            ...
            logger_1.logger.error('Failed to update notification template', {
                error: error instanceof Error ? error.message : 'Unknown error',
                templateId,
                updates,
            });
    ...
    async deleteTemplate(templateId) {
        try {
            ...
            logger_1.logger.info('Notification template deleted', { templateId });
        }
        catch (error) {
            logger_1.logger.error('Failed to delete notification template', {
                error: error instanceof Error ? error.message : 'Unknown error',
                templateId,
            });
            throw error;
        ...
        for (const templateData of defaultTemplates) {
            try {
                ...
                if (!existing) {
                    ...
                    logger_1.logger.info('Default template created', { name: templateData.name });
                }
            }
            catch (error) {
                logger_1.logger.error('Failed to create default template', {
                    name: templateData.name,
                    error: error instanceof Error ? error.message : 'Unknown error',
                });
            }
        }
        logger_1.logger.info('Default templates initialization completed');
    }
}
