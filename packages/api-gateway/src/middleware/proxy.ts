...
export function proxyMiddleware(serviceConfig: ServiceConfig) {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const targetUrl = `${serviceConfig.url}${req.path}`;

      // Prepare headers to forward
      const requestId: string = req.get('X-Request-ID') || 'unknown';
      const remoteAddr: string = req.ip || (req.socket.remoteAddress ?? 'unknown');
...
export function healthCheckProxy(serviceConfig: ServiceConfig) {
...
export function circuitBreakerMiddleware(
  ...
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (circuitOpen && now - lastFailureTime > recoveryTimeout) {
      circuitOpen = false;
      failureCount = 0;
      logger.info('Circuit breaker reset', { service: serviceConfig.url });
    }
