...
import { User, Role, Permission, UserRole } from '@voxlink/shared';
...
export class RBACService {
  ...
  async createRole(
    ...
    try {
      ...
      logger.info(`Role created: ${role.id} - ${name}`);
      return role;
    } catch (error) {
      logger.error('Failed to create role:', { error } as any);
      throw error;
    }
  }
  ...
  async updateRolePermissions(roleId: string, permissions: Permission[]): Promise<Role> {
    try {
      ...
      logger.error('Failed to update role permissions:', { error } as any);
      throw error;
    }
  }
  ...
  async assignRoleToUser(userId: string, role: UserRole): Promise<void> {
    try {
      ...
      logger.error('Failed to assign role to user:', { error } as any);
      throw error;
    }
  }
  ...
  async hasPermission(
    ...
    try {
      ...
      logger.error('Failed to check user permission:', { error } as any);
  ...
  async hasAnyPermission(
    ...
    try {
      ...
      logger.error('Failed to check any permission:', { error } as any);
      return false;
    }
  }
  ...
  async hasAllPermissions(
    ...
    try {
      ...
      logger.error('Failed to check all permissions:', { error } as any);
      return false;
    }
  }
  ...
  async getUserPermissions(userId: string): Promise<Permission[]> {
    try {
      ...
      logger.error('Failed to get user permissions:', { error } as any);
      return [];
    }
  }
  ...
  async getRolePermissions(role: UserRole): Promise<Permission[]> {
    try {
      ...
      logger.error('Failed to get role permissions:', { error } as any);
      return [];
    }
  }
  ...
  async initializeSystemRoles(): Promise<void> {
    try {
      ...
      logger.error('Failed to initialize system roles:', { error } as any);
      throw error;
    }
  }
   ...
   */
  private async invalidateUserPermissionCache(roleId: string): Promise<void> {
    // This would invalidate cache for all users with this role
    // For now, we'll just log it
    logger.debug(`Invalidating permission cache for role: ${roleId}`);
  }
  ...
  private async storeRole(role: Role): Promise<void> {
    logger.debug(`Storing role: ${role.id}`);
  }
  ...
  private async getPermissionsForRole(role: UserRole): Promise<Permission[]> {
