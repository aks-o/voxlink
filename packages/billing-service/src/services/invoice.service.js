...
class InvoiceService {
    constructor(prisma, costCalculator, usageTracking, pdfGenerator) {
        this.prisma = prisma;
    ...
    async generateInvoice(input) {
        ...
        try {
            ...
            const invoiceDueDate = dueDate || new Date(Date.now() + config_1.config.billing.invoiceDueDays * 24 * 60 * 60 * 1000);
            // Generate invoice number
            const invoiceNumber = await this.generateInvoiceNumber();
            // Get uninvoiced usage events for the period
            const usageEvents = await this.usageTracking.getUninvoicedUsage(billingAccountId, periodStart, periodEnd);
            // Create invoice
     ...
     */
    async createInvoiceItems(invoiceId, usageEvents) {
     ...
     */
    async getInvoicesForAccount(billingAccountId, limit = 50, offset = 0) {
    ...
    async markInvoiceAsPaid(invoiceId) {
        ...
        logger_1.logger.info('Invoice marked as paid', {
            invoiceId,
    ...
    async generateInvoicePDF(invoiceId) {
        ...
        logger_1.logger.info('Invoice PDF generated', {
            invoiceId,
            invoiceNumber: invoice.invoiceNumber,
            pdfUrl,
        });
    ...
    async markOverdueInvoices() {
        ...
        logger_1.logger.info('Invoices marked as overdue', {
            count: result.count,
        });
        return result.count;
    }
