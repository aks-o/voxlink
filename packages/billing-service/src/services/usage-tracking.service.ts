...
export class UsageTrackingService {
   ...
   * Track a usage event
   */
  async trackUsage(input: TrackUsageInput): Promise<UsageEvent> {
    const {
      billingAccountId,
      numberId,
      eventType,
    ...
    try {
      ...
      const costResult = await this.costCalculator.calculateUsageCost({
        eventType,
        duration,
        quantity,
        metadata,
   ...
   */
  async markAsInvoiced(usageEventIds: string[], invoiceItemId: string): Promise<void> {
    ...
    logger.info('Usage events marked as invoiced', {
   ...
   */
  async getDailyUsageAggregation(
    billingAccountId: string,
    startDate: Date,
    endDate: Date
  ): Promise<Array<{ date: string; totalCost: number; eventCount: number }>> {
    ...
    // Group by date
    const dailyAggregation = new Map<string, { totalCost: number; eventCount: number }>();
   ...
   */
  async trackMonthlySubscription(billingAccountId: string, numberId: string): Promise<UsageEvent> {
   ...
   */
  async trackSetupFee(billingAccountId: string, numberId: string): Promise<UsageEvent> {
    return this.trackUsage({
      billingAccountId,
      numberId,
      eventType: 'SETUP_FEE',
